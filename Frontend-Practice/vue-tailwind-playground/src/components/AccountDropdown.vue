<template>
  <div class="hidden sm:block relative">
    <button @click="isOpen = !isOpen" class="relative z-20 block h-8 w-8 rounded-full overflow-hidden border-2 border-gray-600 focus:outline-none focus:border-white">
    ...
    <button v-if="isOpen" @click="isOpen = false" tabindex="-1" class="fixed z-20 h-full w-full inset-0 bg-black opacity-50 cursor-default"></button>
    <div v-if="isOpen" class="absolute z-30 right-0 mt-2 py-2 w-48 bg-white rounded-lg shadow-xl">
...
<script>
  ...
  name: 'AccountDropdown',
    ...
    mounted() {
      ...
      onUnmounted(() => document.body.removeEventListener('keydown', handleEscape));
    }
  });
</script>